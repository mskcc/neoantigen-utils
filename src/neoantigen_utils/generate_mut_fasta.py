...
import pandas as pd
import logging
# mutalyzer is a heavy, container-provided runtime dependency. Import it
# lazily-tolerantly so the module (and its unit tests) import cleanly on hosts
# without it; ``normalize`` stays a module-level name so tests can monkeypatch it
# and the container's real implementation is used at runtime.
try:
    from mutalyzer.normalizer import normalize
except ImportError:  # pragma: no cover - exercised only where mutalyzer is absent
    normalize = None
