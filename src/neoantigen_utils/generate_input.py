...
def get_exons_from_transcriptID(transcriptid, ensembl, complete=True):
    ...
    :param ensembl: Genome instance in pyensembl
    :param complete: only consider complete transcripts
    :return: a list of (start, end) genomic exon ranges, ordered 5'->3'
             (transcript order). start <= end for every exon.

    pyensembl's ``Transcript.exons`` yields exons already ordered from the
    5' to the 3' end of the transcript (so for a minus-strand transcript the
    genomic coordinates run high->low across the list). Using that ordering is
    required for spliced-coordinate math to be correct on both strands.
    """
    transcript = ensembl.transcript_by_id(_strip_transcript_version(transcriptid))
    return [(exon.start, exon.end) for exon in transcript.exons]


def _strip_transcript_version(transcriptid):
    """ENST ids in a MAF may carry a version suffix (e.g. ENST0000... .3).
    pyensembl indexes them without the version, so drop it if present."""
    if transcriptid is None:
        return None
    return str(transcriptid).split(".")[0]


def _exon_len(exon):
    """Length of an exon in nt. Genomic ranges are inclusive on both ends,
    so the length is end - start + 1 (fixes the off-by-one in the original)."""
    return exon[1] - exon[0] + 1


def _spliced_offset(exons, strand, genomic_pos):
    """0-based offset of ``genomic_pos`` along the spliced transcript, measured
    from the transcript 5' end. Returns None if the position is intronic /
    outside every exon. Strand-aware: on the minus strand the 5' end of an exon
    is its high genomic coordinate.
    """
    cum = 0
    for (s, e) in exons:
        if s <= genomic_pos <= e:
            if strand == "-":
                return cum + (e - genomic_pos)
            return cum + (genomic_pos - s)
        cum += (e - s + 1)
    return None


def _locate_exon(exons, spliced_offset):
    """Given a spliced offset (5'->3'), return (exon_index, exon_start_offset)
    for the exon that contains it. If the offset is past the 3' end of the
    transcript (e.g. a PTC that would fall beyond the annotated exons) the last
    exon is returned."""
    cum = 0
    for i, (s, e) in enumerate(exons):
        length = e - s + 1
        if spliced_offset < cum + length:
            return i, cum
        cum += length
    last_len = _exon_len(exons[-1])
    return len(exons) - 1, cum - last_len


def extract_nmd_features(exons, strand, mut_genomic_pos, mut_to_stop_dist,
                         start_codon_offset=0):
    """Feature extraction for the NMDetective-B decision tree, in spliced
    coordinates with full strand handling. Pure function of its inputs so it
    can be unit-tested without pyensembl.

    :param exons: list of (start, end) genomic ranges ordered 5'->3'
    :param strand: '+' or '-'
    :param mut_genomic_pos: genomic position of the causal mutation
    :param mut_to_stop_dist: nt downstream (spliced/CDS space) of the mutation
                             at which the premature stop (PTC) sits
    :param start_codon_offset: spliced offset (from transcript 5' end) of the
                               start codon; used to measure distance of the PTC
                               from the start codon along the spliced CDS
    :return: dict of the four classifier features, or None if the mutation
             cannot be localized to an exon (unlocalizable PTC)
    """
    mut_offset = _spliced_offset(exons, strand, int(mut_genomic_pos))
    if mut_offset is None:
        # Mutation is not inside any exon -> we cannot place the PTC.
        return None

    # The PTC sits mut_to_stop_dist nt downstream of the mutation in spliced
    # (CDS-colinear) space.
    ptc_offset = mut_offset + mut_to_stop_dist

    ptc_idx, ptc_exon_start_offset = _locate_exon(exons, ptc_offset)
    last_idx = len(exons) - 1

    is_last_exon = (ptc_idx == last_idx)

    # Distance of the PTC from the start codon along the spliced CDS.
    dist_from_start_nt = ptc_offset - start_codon_offset

    ptc_exon_len_nt = _exon_len(exons[ptc_idx])

    # Distance from the PTC to the final exon-exon junction (the 3' end of the
    # penultimate exon). The "50 nt rule" only applies when the PTC lies in the
    # penultimate exon and is within 50 nt (upstream) of that final junction.
    ptc_exon_end_offset = ptc_exon_start_offset + ptc_exon_len_nt - 1
    dist_to_final_junction = ptc_exon_end_offset - ptc_offset
    within_50nt_of_last_junction = (
        ptc_idx == last_idx - 1 and 0 <= dist_to_final_junction <= 50
    )

    return {
        "is_last_exon": is_last_exon,
        "dist_from_start_nt": dist_from_start_nt,
        "ptc_exon_len_nt": ptc_exon_len_nt,
        "within_50nt_of_last_junction": within_50nt_of_last_junction,
    }


def classify_nmd(is_last_exon, dist_from_start_nt, ptc_exon_len_nt,
                 within_50nt_of_last_junction):
    """Pure NMDetective-B decision tree (Lindeboom et al. 2019).

    Four nested tests, in order, first match wins:
      1. PTC in the last exon                       -> "Last Exon"
      2. else < 150 nt from the start codon         -> "Start-proximal"
      3. else in an exon longer than 407 nt         -> "Long Exon"
      4. else within 50 nt of the final junction    -> "50nt Rule"
      5. else                                        -> "Trigger NMD"
    """
    if is_last_exon:
        return "Last Exon"
    if dist_from_start_nt < 150:
        return "Start-proximal"
    if ptc_exon_len_nt > 407:
        return "Long Exon"
    if within_50nt_of_last_junction:
        return "50nt Rule"
    return "Trigger NMD"


def _get_transcript_model(chrom, pos, ensembl, transcriptID):
    """Resolve exons (5'->3'), strand and start-codon spliced offset for the
    relevant transcript. Prefers the annotated transcriptID; otherwise falls
    back to the longest complete transcript at the locus. Returns
    (exons, strand, start_codon_offset) or (None, None, None) if no transcript
    can be resolved."""
    if transcriptID is None:
        transcript = get_transcript(chrom, pos, ensembl)
    else:
        transcript = ensembl.transcript_by_id(
            _strip_transcript_version(transcriptID)
        )
    ...
    try:
        ...
        # Non-coding / incomplete transcript: fall back to transcript 5' end.
        start_codon_offset = 0
    return exons, strand, start_codon_offset
...
def determine_NMD(chrom, pos, num_windows, len_indel, ensembl, transcriptID=None):
    ...
    :param transcriptID: transcriptID from the MAF. If it isn't annotated, use
                         the longest complete transcript at the locus.
    :param num_windows: number of windows created by the peptide
    :param len_indel: length of the indel. Negative if del, positive if ins
    :param ensembl: Genome/EnsemblRelease instance in pyensembl
    :return: NMDetective-B classification string, or "False" if the PTC cannot
             be localized.

    The premature stop codon (PTC) sits ``num_windows*3 + len_indel + 1`` nt
    downstream (in spliced/CDS space) of the mutation. This meaning is
    preserved from the original implementation.
    """
    mut_to_stop_dist = (num_windows * 3) + len_indel + 1

    try:
        exons, strand, start_codon_offset = _get_transcript_model(
            chrom, pos, ensembl, transcriptID
        )
    except Exception:
        return "False"
    ...
    features = extract_nmd_features(
        exons, strand, pos, mut_to_stop_dist, start_codon_offset
    )
    if features is None:
        # PTC could not be localized to an exon.
        return "False"

    return classify_nmd(**features)
